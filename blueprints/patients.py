...
def get_students_cached():
    return query("SELECT id, username FROM users WHERE role = %s ORDER BY username ASC", (Role.ALUNO,))
...
def sign_student_atendimento(id, appt_id):
    ...
    try:
        ...
        flash('Assinatura do aluno (executor) registrada com sucesso!', 'success')
    except Exception as e:
        flash(f'Erro ao registrar assinatura: {str(e)}', 'danger')
...
def sign_atendimento(id, appt_id):
    ...
    try:
        ...
        flash('Evolução clínica assinada pelo professor com sucesso!', 'success')
    except Exception as e:
...
def validate_exam(id, exam_id):
    ...
    # We allow role 'professor' or 'admin' to validate
    if not prof or not check_password_hash(prof['password'], password) or prof['role'] not in [Role.PROFESSOR, Role.ADMIN]:
        flash('Credenciais inválidas ou usuário sem permissão de professor.', 'danger')
        return redirect(url_for('patients.view_patient', id=id))
    ...
    try:
        execute('''
            ...
            WHERE id = %s AND patient_id = %s
        ''', (prof['id'], exam_id, id))
        flash('Exame validado pelo professor com sucesso!', 'success')
    except Exception as e:
